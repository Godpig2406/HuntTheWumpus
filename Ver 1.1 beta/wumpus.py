...
What do you want to do? (M)ove/(S)hoot? """)
    ...
    def sense(self):
        ...
        # Give hints, but avoids showing the same hint multiple times.
        repeats = [False for i in range(3)]
        for i in Cave[self.location].link:
            if Cave[i].type == "wumpus" and not repeats[0]:
                show("You smell something terrible nearby")
                repeats[0] = True
            elif Cave[i].type == "pit" and not repeats[1]:
                show("You feel a cold wind blowing from a nearby cavern.")
                repeats[1] = True
            elif Cave[i].type == "bat" and not repeats[2]:
                show("You hear a rustling sound nearby")
                repeats[2] = True
    ...
    def shoot(self, to):
        if to in Cave[self.location].link:
            ...
            if Cave[to].type == "wumpus":
                show("YOU KILLED THE WUMPUS! GOOD JOB, BUDDY!!!")
                Playing = False
            else:
                show(f"You missed!\nYou have {str(self.arrow)} arrows left.")
                if room.move_wumpus():
                    # The wumpus moved to player's room
                    show("You woke up the wumpus......")
                    Playing = False
                else:
                    # The wumpus moves to another room
                    show("You heard a rumbling in a nearby cavern.")
        else:
            show("Not in range")
    ...
    def check(self):
        ...
        if Cave[self.location].type == "wumpus":
            Playing = False
            show("The wumpus ate you up!")
        elif Cave[self.location].type == "pit":
            Playing = False
            show("You fell into a bottomless pit. Enjoy the ride!")
        elif Cave[self.location].type == "bat":
            self.location = random.randint(0, 19)
            show("The bats whisk you away!")
            self.check()
